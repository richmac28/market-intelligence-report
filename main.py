...
class MarketDataCollector:
        ...
        for symbol, name in symbols.items():
            try:
                ...
                if response.status_code == 200:
                    ...
                    result[name] = {'value': price, 'source': 'Bloomberg'}
                    logger.info(f"OK {name}: {price}")
    ...
    def get_nifty_sensex(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if records:
                    ...
                    logger.info(f"OK Nifty: {nifty_value}")
        except Exception as e:
            logger.error(f"FAIL Nifty: {str(e)}")
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.info(f"OK Sensex: {sensex_value}")
        except Exception as e:
            logger.error(f"FAIL Sensex: {str(e)}")
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if records:
                    ...
                    logger.info(f"OK VIX: {vix_value}")
        except Exception as e:
            logger.error(f"FAIL VIX: {str(e)}")
    ...
    def get_commodities(self):
        ...
        symbols = {'NYMEX:CL1!': 'Crude Oil', 'NYMEX:GC1!': 'Gold'}
        for symbol, name in symbols.items():
            try:
                ...
                if response.status_code == 200:
                    ...
                    if 'c' in data:
                        ...
                        result[name] = {
                            'value': round(price, 2),
                            'change_pct': f"{data.get('dp', 0):+.2f}%",
                            'source': 'Finnhub'
                        }
                        logger.info(f"OK {name}: {price:.2f}")
            except Exception as e:
                logger.error(f"FAIL {name}: {str(e)}")
                result[name] = {'value': 'N/A', 'source': 'Error'}
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if data.get('observations'):
                    ...
                    logger.info(f"OK 10Y: {yield_value:.2f}%")
        except Exception as e:
            logger.error(f"FAIL Yield: {str(e)}")
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if data_list:
                    ...
                    logger.info(f"OK FII/DII")
        except Exception as e:
            logger.error(f"FAIL FII/DII: {str(e)}")
...
def generate_html_email(market_data):
    ...
    <tr><td>S&P 500</td><td>{us_indices.get('S&P 500', {}).get('value', 'N/A')}</td><td>{us_indices.get('S&P 500', {}).get('source', 'N/A')}</td></tr>
    <tr><td>Nasdaq</td><td>{us_indices.get('Nasdaq 100', {}).get('value', 'N/A')}</td><td>{us_indices.get('Nasdaq 100', {}).get('source', 'N/A')}</td></tr>
    <tr><td>Gold</td><td>{commodities.get('Gold', {}).get('value', 'N/A')}</td><td>{commodities.get('Gold', {}).get('source', 'N/A')}</td></tr>
    </table>
    <h2>India Markets</h2>
    <table border="1" cellpadding="10" style="width:100%;border-collapse:collapse;">
    <tr><th>Index</th><th>Value</th><th>Source</th></tr>
    <tr><td>Nifty 50</td><td>{market_data.get('Nifty 50', {}).get('value', 'N/A')}</td><td>{market_data.get('Nifty 50', {}).get('source', 'N/A')}</td></tr>
    <tr><td>Sensex</td><td>{market_data.get('Sensex', {}).get('value', 'N/A')}</td><td>{market_data.get('Sensex', {}).get('source', 'N/A')}</td></tr>
    <tr><td>VIX</td><td>{market_data.get('india_vix', {}).get('value', 'N/A')}</td><td>{market_data.get('india_vix', {}).get('source', 'N/A')}</td></tr>
    </table>
    </div>
    <div style="background:#1a1f3a;color:white;padding:20px;text-align:center;">
    <p>Automated Daily Report</p>
    <p>Data from: Bloomberg, Moneycontrol, NSE, FRED, Finnhub</p>
    </div>
    </body></html>"""
    return html
